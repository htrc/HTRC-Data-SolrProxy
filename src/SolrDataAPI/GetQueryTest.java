...
public class GetQueryTest {
	...
	public StreamingOutput getQueryString(
		...
		if (!uri.toString().startsWith("http://coffeetree.cs.indiana.edu:9994/solr/select/")) {
			
			Date today = new Date();
			String log_content = "\n"+ hsr.getHeader("x-forwarded-for") +"	"+ hsr.getRemoteAddr() + "	" + today.toString() + "	"+ uri+"	"+"blocked";
			System.out.println(log_content);		
			RandomAccess.writeLog(logfile.getAbsolutePath(), log_content);
		...
		String solr_endpoint = (uri.toString().replace(
				"http://coffeetree.cs.indiana.edu:9994", "http://coffeetree.cs.indiana.edu:9992"));
		...
		*/
		...
		return new StreamingOutput() {
			...
			public void write(OutputStream arg0) throws IOException,
				...
				String log_content = "\n"+ hsr.getHeader("x-forwarded-for") +"	"+ hsr.getRemoteAddr() + "	" + today.toString() + "	"+ uri;
				System.out.println(log_content);		
				RandomAccess.writeLog(logfile.getAbsolutePath(), log_content);
				...
				//System.out.println("time consumed to download is: " + (t4 - t3));

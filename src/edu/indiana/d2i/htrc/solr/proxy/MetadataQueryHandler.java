...
public class MetadataQueryHandler {
	...
	@Context UriInfo ui, @Context final HttpServletRequest hsr) {
		...
		debugLogger.info("hostname: " + (ParamDef.getConfig().getProperty("solr.meta.host")));
		debugLogger.info("port: " + (ParamDef.getConfig().getProperty("solr.meta.port")));
		uriBuilder.host(ParamDef.getConfig().getProperty("solr.meta.host")).port(Integer.valueOf(ParamDef.getConfig().getProperty("solr.meta.port")))
				.replacePath("solr/" + ParamDef.getConfig().getProperty("solr.meta.core") + "/select");

		URI newURI = uriBuilder.build();
		debugLogger.info("uriBuilder.build(): " + newURI);
